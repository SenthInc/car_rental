...
export default function Filter({ setFilteredCars, cars, setLoading }: Props) {
  ...
  useEffect(() => {
    ...
    // Check if cars is null or empty
    if (!cars || cars.length === 0) {
      setFilteredCars([]);  // Return an empty array if cars are empty or undefined
      if (setLoading) {
        setLoading(false);
      }
      return;
    ...
    // Set filtered cars or empty array
    if (newCars.length < cars.length) {
      setFilteredCars(newCars);
    } else {
      setFilteredCars([]);  // Set to empty array instead of null
    }
